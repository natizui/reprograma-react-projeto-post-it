...
class LoginForm extends React.Component {
    ...
    handleChange(e) {
        ...
        if (nomeDoInput === 'nome' && valorDoInput.trim() === '') { //.trim() tira espaços em branco
            this.setState({ 
                erro: { 
                    tipo: "nome",
        ...
        if (nomeDoInput === 'nome' && valorDoInput.length < 10) { 
            this.setState({ erro: {tipo: "nome", mensagem: "Digite no mínimo 10 caracteres"} })
            return
        }
        ...
        if (nomeDoInput === 'senha' && valorDoInput.length < 6) { 
            this.setState({ erro: {tipo: "senha", mensagem: "Digite no mínimo 6 caracteres"} })
            return
        }
